        ...
        private void btn_calculation_Click(object sender, EventArgs e)
        {
            if (Check())
            {
                ...
                switch (cmb_duration.SelectedIndex)
                {
                    case 0:
                        {
                            allPeriods = Int32.Parse(txt_duration.Text);
                            cmb_periods.Enabled = false;
                            break;
                        }
                    case 1:
                        {
                            allPeriods = Int32.Parse(txt_duration.Text) * 12;
                            cmb_periods.Enabled = true;
                            break;
                        }
                }
                ...
                {
                    payment = ((double)credit_sum / allPeriods) + ostatok * ((double)procent / annual_period);
                    allPays += payment;
                    overPay = ostatok * ((double)procent / annual_period);
                    all_over += overPay;
                    ...
                    pay_schedule.Rows.Add(
                        new string[5] {
                            ...
                            Math.Round(overPay,2).ToString(),
                            Math.Round((double)credit_sum / allPeriods,2).ToString(),
                ...
                chart_results.Series[0].Points.AddY(Math.Round(overPay,2));
                chart_results.Series[0].Points[1].LegendText = "Переплата";
                lbl_results_procent.Text = Math.Round(overPay, 2).ToString();
                lbl_results_all.Text = (credit_sum + Math.Round(overPay, 2)).ToString();
